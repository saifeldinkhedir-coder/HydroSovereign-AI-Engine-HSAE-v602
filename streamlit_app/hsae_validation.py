...
""", unsafe_allow_html=True)
    ...
    # ── Independence requirement (peer-review objection #5) ───────────────
    st.warning(
        "**Benchmark independence is required.** A meaningful skill score "
        "(NSE/KGE) is only valid when the observed benchmark is independent "
        "of the model's own forcing. A 'GloFAS' series derived from the same "
        "precipitation (e.g. GPM) that forces the model measures shared "
        "input, not skill. Upload **observed gauge discharge (e.g. GRDC)** "
        "that does not share the model's forcing source.", icon="⚠️")
